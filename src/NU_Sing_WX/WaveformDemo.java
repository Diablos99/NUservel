...
{	
    ...
    //Slider
	static Timer timer; // Updates slider every 100 milliseconds
    ...
    private static JLabel fileLabel = new JLabel("No file loaded");
    private DisplayPanel displayPanel = new DisplayPanel();
    static JToolBar playbackTools = new JToolBar();
    
    private ToolsButton bOpen = new ToolsButton("Open");
    protected static ToolsButton playWav = new ToolsButton("Play");
    private ToolsButton bPlay = new ToolsButton("PlayWaveForm");
    ...
    private volatile static PlayStat playStat = PlayStat.NO_FILE;
    ...
    public float getVolume() {
        FloatControl gainControl = (FloatControl) NU_MainFrame.clip.getControl(FloatControl.Type.MASTER_GAIN);        
    ...
    static void loadAudio(String wavepath) throws LineUnavailableException {            
        
            File selected = new File(wavepath);
            
            try {                               
                AudioFileFormat fmt = AudioSystem.getAudioFileFormat(selected);               
                audioFile = selected;
                audioFormat = fmt.getFormat();
                AudioInputStream ain = AudioSystem.getAudioInputStream(selected);
    			try {
    				DataLine.Info info = new DataLine.Info(Clip.class, ain
    						.getFormat());
    				NU_MainFrame.clip = (Clip) AudioSystem.getLine(info);
    				NU_MainFrame.clip.open(ain);
    			} finally { // We're done with the input stream.
    				ain.close();
    			}
    			NU_MainFrame.audioLength = (int) (NU_MainFrame.clip.getMicrosecondLength() / 1000);
